...
describe("calculateCartSummary", () => {
  it("computes tax-exclusive math (tax added on top of subtotal)", () => {
    ...
    // Feature 28B — exact fixed-decimal strings, compared exactly. toBeCloseTo
    // was hiding precisely the cent of drift this feature removes.
    expect(result.subtotal).toBe("8.00");
    expect(result.taxAmount).toBe("0.80");
    expect(result.total).toBe("8.80");
  });
  ...
  it("computes tax-inclusive math (tax extracted from subtotal, total unchanged)", () => {
    ...
    expect(result.subtotal).toBe("8.00");
    // 8 - 8/1.1 = 0.7272..., which rounds to 0.73.
    expect(result.taxAmount).toBe("0.73");
  ...
  it("never fabricates a tip from a non-finite or negative tipAmount", () => {
    expect(calculateCartSummary([TAXED_ITEM], makeTax(), Number.NaN).tip).toBe("0.00");
    expect(calculateCartSummary([TAXED_ITEM], makeTax(), -5).tip).toBe("0.00");
    expect(calculateCartSummary([TAXED_ITEM], makeTax(), Infinity).tip).toBe("0.00");
    // And the RPC argument follows the displayed value, never diverges from it.
    expect(calculateCartSummary([TAXED_ITEM], makeTax(), Number.NaN).tipAmount).toBe(0);
  });
  ...
  it("passes through a real explicit positive tip", () => {
    ...
    expect(result.tipAmount).toBe(3);
    expect(result.total).toBe("11.80");
  });
